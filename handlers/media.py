...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔀 2. УМНАЯ МАРШРУТИЗАЦИЯ ПО РЕЖИМАМ
    if mode == 'openclaw_wait':
        ...
        # Сохраняем для модуля Image-to-Video
    ...
    # --- 3. ЕСЛИ РЕЖИМА НЕТ (Стандартное меню Vision / Upscale) ---
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    🔥 НОВЫЙ ПЕРЕХВАТЧИК: Ловит PDF, Excel, TXT, CSV и передает Агенту
    """
    user_id = update.effective_user.id
    mode = context.user_data.get('mode')
    ...
    # Убираем пробелы и спецсимволы из имени файла для безопасности bash
    safe_name = "".join(c for c in document.file_name if c.isalnum() or c in " ._-")
    
    # Скачиваем файл и получаем его АБСОЛЮТНЫЙ ПУТЬ
