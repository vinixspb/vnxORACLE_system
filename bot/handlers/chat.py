...
# Просьбы показать интерфейс обрабатываем кодом: модель не умеет рисовать
# клавиатуру и вместо этого пересказывает меню прозой, выдумывая команды.
#
# Срабатывание требует намерения И объекта, иначе «расскажи про кнопки в нашем
# боте поддержки» открывало бы меню вместо ответа по делу.
_MENU_INTENT = ("покажи", "показать", "покаж", "открой", "открыть", "дай",
                "выведи", "верни", "где", "хочу", "нужно", "какие", "что",
                "чем", "show", "open")
_MENU_OBJECT = ("меню", "кнопк", "клавиатур", "menu", "button", "keyboard",
                "возможност", "функци", "команд", "раздел", "навигаци")

# Самодостаточные фразы: намерение и объект слиты, разбирать нечего.
_MENU_EXACT = frozenset({
    "меню", "кнопки", "кнопка", "клавиатура", "menu", "buttons", "keyboard",
    "помощь", "help", "справка", "начать", "start", "настройки", "settings",
    "что ты умеешь", "что умеешь", "что ты можешь", "что можешь",
    "твои возможности", "какие возможности", "возможности", "функции",
    "команды", "какие команды", "что дальше", "с чего начать",
    "what can you do", "options",
})


def is_menu_request(text: str) -> bool:
    """True, если пользователь просит показать интерфейс, а не обсудить его."""
    if not text:
        return False

    low = " ".join(text.lower().strip(" .!?…").split())
    if low in _MENU_EXACT:
        return True

    # Длинная фраза — это разговор о кнопках, а не просьба их показать
    if len(low.split()) > 4:
        return False

    return any(obj in low for obj in _MENU_OBJECT) and any(
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_menu_request(text):
        ...
        await update.message.reply_text(
            "⌨️ <b>Панель управления</b> — под полем ввода.",
